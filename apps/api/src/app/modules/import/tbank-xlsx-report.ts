 ...
 *    (Сумма БЕЗ НКД) / Количество — общая формула, верная и для акций/фондов
 *    (для них совпадает с «Цена за единицу» дословно, т.к. НКД там всегда 0).
 *    Намеренно НЕ «Сумма сделки» (=Сумма без НКД + НКД) — НКД, уплаченный
 *    продавцу при покупке облигации, экономически не часть цены/себестоимости:
 *    он возвращается покупателю следующим купоном (который выплачивается за
 *    весь период целиком, а не только за время фактического владения). Если
 *    включить НКД в цену покупки, средняя цена будет задрана относительно
 *    текущей котировки MOEX (та тоже чистая, без НКД — moex.provider.ts),
 *    и P&L по облигации будет систематически занижен.
...
const TRADE_COL = {
  ...
  settleCcy: 39,
  qty: 27,
  /** «Сумма без НКД» — БЕЗ накопленного купонного дохода (см. пояснение выше) */
  amountExNkd: 29,
  brokerFee: 40,
  exchangeFee: 46,
  clearingFee: 51,
...
function parseTradesSection(ws: ExcelJS.Worksheet, from: number, to: number): RawRow[] {
  ...
  for (let r = from; r <= to; r++) {
    ...
    const amountExNkd = cellNum(ws, r, TRADE_COL.amountExNkd);
    // фактическая цена за единицу в деньгах, БЕЗ НКД — не «Цена за единицу»
    // (для облигаций это %) и не «Сумма сделки» (та включает НКД, см. пояснение выше)
    const price = qty !== 0 ? new Decimal(amountExNkd).div(qty).toString() : '0';
